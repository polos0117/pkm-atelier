...
(async()=>{
 ...
 try{
  ...
  assert(a.get('feature-inset-front').value.includes('plastron chest plate'));
  assert(a.get('feature-inset-rear').value.includes('seed bulb'));
  assert(a.get('feature-inset-function').value.includes('vine cable emitters'));
  await a.set('feature-inset-front','segmented chest reactor with luminous channels');
  assert(a.output().includes('segmented chest reactor with luminous channels'));
  ...
  assert(a.output().includes('U-shaped lower-face outline'));
  assert(a.get('feature-insets'));
  assert.equal(a.get('feature-inset-front').value,'segmented chest reactor with luminous channels');
  await pause();
  const state=a.w.localStorage.getItem(storeKey);
  assert.equal(JSON.parse(state).cards['이상해씨'].params['hair length'],'medium');
  assert.equal(JSON.parse(state).cards['이상해씨'].featureInsets.front,'segmented chest reactor with luminous channels');
