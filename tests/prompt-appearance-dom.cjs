...
(async()=>{
 ...
 try{
  ...
  assert(a.get('feature-insets'),'create portrait needs feature-inset editor');
  await a.set('motifs','green palette, segmented plastron chest plate, dorsal seed bulb, vine cable emitters');
  assert(a.get('feature-inset-mount').value.includes('seed bulb'),'a rooted motif feeds the rear-mount inset');
  assert(!a.get('feature-inset-mount').value.includes('plastron'),'a front plate is not a rear mount');
  assert(a.get('feature-inset-design').value.includes('shoulder plate'));
  await a.set('feature-inset-design','segmented shoulder reactor with luminous channels');
  assert(a.output().includes('segmented shoulder reactor with luminous channels'));
  ...
  assert(a.output().includes('species head feature treatment: tiny leaf clips'));
  await a.set('head-feature','accessory');
  ...
  assert(!a.output().includes('[FEATURE INSET LIST — FIXED]'));
  ...
  assert(a.output().includes('U-shaped lower-face outline'));
  assert(a.get('feature-insets'));
  assert.equal(a.get('feature-inset-design').value,'segmented shoulder reactor with luminous channels');
  ...
  assert.equal(JSON.parse(state).cards['이상해씨'].featureInsets.design,'segmented shoulder reactor with luminous channels');
