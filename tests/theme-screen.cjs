...
(async()=>{
 ...
 try{
  ...
  for(const [page,selector] of [['dex.html','.grid .cell'],['prompt.html','#prompt-output'],['battle.html','.bt-setup'],['run.html','.run-draft'],['index.html','.theme-card']]){
   ...
   assert.deepEqual(nav.map(x=>x.href),['index.html','dex.html','prompt.html','battle.html','run.html'],page+' common navigation');
