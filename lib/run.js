/* 런 — 일곱 자리가 한 팩에서 뽑고, 나는 그 여섯과 보스를 차례로 만난다.
   ...
   상태는 JSON 그대로다 (localStorage 에 넣는 것은 화면의 일).
     { seed, phase: 'draft'|'fight'|'reward'|'won'|'lost',
       round, turn, order: [[자리…] × ROUNDS], pack: [{ name, by }], rerolls,
       seats: [{ pref, roster: [{ name, origin }] }],   0 번이 나
       roster: [{ name, hp, origin, grown }],           hp 0~1, 0 이면 쓰러진 것
...
(function (root, factory) {
  ...
  function beginRun(state, by) {
    var i;
    state.roster = state.seats[0].roster.map(function (r) { return { name: r.name, hp: 1, origin: r.origin, grown: 0 }; });
    var ais = [];
    for (i = 1; i < SEATS; i++) ais.push(i);
    ais.sort(function (a, b) { return power(by, state.seats[a].roster) - power(by, state.seats[b].roster) || a - b; });
    state.foes = ais;
    state.phase = 'fight'; state.stage = 1;
    return state;
  }
  ...
  function grownRoster(state, by, seat, times) {
    ...
    for (i = 0; i < state.seats[seat].roster.length; i++) {
      ...
      out.push({ name: name, origin: r.origin, grown: grown });
    }
    return out;
  }
  function encounter(state, cards) {
    ...
    return { seat: -1, members: names.map(function (n) { return { name: n, origin: 0, grown: 0 }; }), policy: 'managed', seed: fightSeed(state) };
  }
  ...
  function actors(state, cards, chart, policy) {
    ...
    for (i = 0; i < state.roster.length; i++) {
      r = state.roster[i];
      if (r.hp <= 0) continue;
      a.push(B.makeActor(by[r.name], { side: 'a', policy: policy || 'managed', remaining: left(by, r.name), hpFrac: r.hp, tuning: budgetOf(r.grown) }));
    }
    var scale = DIFF.foeScale[Math.min(state.stage, DIFF.foeScale.length) - 1];
    for (i = 0; i < foe.members.length; i++) {
      m = foe.members[i];
      b.push(B.makeActor(by[m.name], { side: 'b', policy: foe.policy, remaining: left(by, m.name), tuning: budgetOf(m.grown, scale) }));
    }
