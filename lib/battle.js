...
(function (root, factory) {
  ...
  var TUNING = {
    ...
    lightReturnFree: false,                  // 경장 복귀가 행동까지 안 먹나 (신호 8 후보 D)
    open:   { cost: 60, entryHeat: 25, beats: 2, atk: 1.8, coolLock: 3, vent: 40 },
    forced: { atk: 1.2, coolLock: 5,         // 대상 못 고름 · 위력 낮음 · 냉각 김 (규칙)
              vent: 40, stun: false, beats: 0 },   // 끝날 때 빠지는 Heat · 열린 동안 행동을 못 하나 · 지속(0 이면 open.beats) (신호 8 후보)
  ...
  function battle(sideA, sideB, opt) {
    ...
    function endOpen(a) {
      ...
      a.open = null;
      a.heat -= forced ? T.forced.vent : T.open.vent;   // 방출 — 열이 빠진다 (강제는 따로 값)
      if (a.heat < 0) a.heat = 0;
  ...
  var MANAGED = {
    heatOut: 90,       // 이 Heat 부터 경장으로 빠진다 (70 은 너무 일렀다 — 실험 신호 7)
