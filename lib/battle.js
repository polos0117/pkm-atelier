...
(function (root, factory) {
  ...
  /* 성격 — 어느 폼이 몸에 맞나. 포켓몬 성격처럼 두 칸이 오르고 두 칸이 내린다 (예산 합은 거의 그대로).
     고르면 그 폼으로 판에 서고, 운용(managed)이 그 폼을 일하는 폼으로 삼는다. 갈아 입기는 그대로 된다.
     ±10% 로는 맞게 고른 것과 거꾸로 고른 것이 안 갈렸다(tests/run-sim.cjs) — ±20% 로 둔다.
     경장형은 경장에 살지 않는다 — 경장은 식히는 자리라 거기 살면 3% 다. 시작만 경장이고 일하는 폼은 능력치대로 */
  var NATURE = {
    heavy:    { def: 1.2, spd: 1.2, atk: 0.8, spe: 0.8 },   // 중장형 — 받아낸다
    mobility: { atk: 1.2, spe: 1.2, def: 0.8, spd: 0.8 },   // 고기동형 — 피하고 따라붙는다
    light:    { hp: 1.2, spa: 1.2, atk: 0.8, def: 0.8 },    // 경장형 — 되돌린다 (HP 로 오래 버티고 특공으로 때린다)
  };
  function applyNature(st, nature) {
    var m = NATURE[nature], k, out = {};
    for (k in st) out[k] = m && m[k] ? Math.round(st[k] * m[k]) : st[k];
    return out;
  }
  /* 능력치에 가장 맞는 성격 — 오르는 두 칸의 몫이 가장 큰 것 */
  function fitNature(st) {
    var best = null, bs = -1, k, m, s, j;
    for (k in NATURE) {
      m = NATURE[k]; s = 0;
      for (j in m) if (m[j] > 1) s += st[j];
      if (s > bs) { bs = s; best = k; }
    }
    return best;
  }
  ...
  function makeActor(card, opt) {
    ...
    if (opt.nature && NATURE[opt.nature]) st = applyNature(st, opt.nature);
    return {
      nature: opt.nature && NATURE[opt.nature] ? opt.nature : null,
      ...
      stats: st, hpMax: st.hp * T.hpScale,
      hp: Math.round(st.hp * T.hpScale * (opt.hpFrac == null ? 1 : Math.max(0, Math.min(1, opt.hpFrac)))),   // 런은 체력을 비율로 들고 다닌다
      form: opt.form || (opt.nature && NATURE[opt.nature] ? opt.nature : 'light'), open: null,   // 성격이 있으면 그 폼으로 선다
  ...
  /* ── 한 판 ───────────────────────────────────────────
     createBattle 은 판을 세우고 한 박자씩 굴리는 손잡이를 준다 — 화면이 박자 사이에
     멈춰 사람의 예약을 받는 자리다. battle 은 그것을 끝까지 돌린 것이라 결과가 같다 */
  function createBattle(sideA, sideB, opt) {
    opt = opt || {};
    ...
    function pickTarget(a, want) {
      var f = foes(a); if (!f.length) return null;
      if (want) { var i; for (i = 0; i < f.length; i++) if (f[i].name === want) return f[i]; }
    ...
    function doOpen(a, want) {
      ...
      var t = pickTarget(a, want); if (t) hit(a, t, 1);
      return true;
    }
    ...
    function act(a) {
      ...
      else if (cmd.kind === 'open') done = doOpen(a, cmd.target);
      if (!done) { var t = pickTarget(a, cmd.target); if (t) hit(a, t, 1); ev('attack', a); }
      ...
      if (a.alive && a.form === 'mobility' && !a.open && cmd.extra && a.drive >= T.form.mobility.extraCost && (T.overheat.extra || !a.overheated)) {
        ...
        var t2 = pickTarget(a, cmd.target); if (t2) hit(a, t2, 1); ev('extra', a);
      }
    }
    ...
    function step() {
      ...
      if (snaps) snap();
      var s = aliveSides();
      if (!s.a || !s.b || beat >= maxBeats) over = true;
      return over;
    }
    function result() {
      var end = aliveSides();
    ...
    /* 사람이 고를 수 있는 것 — 화면이 단추를 켜고 끄는 근거. 규칙은 여기 한 곳 */
    function can(a) {
      var forms = ['light', 'heavy', 'mobility'].filter(function (f) {
        return f !== a.form && !a.open && a.switchLock === 0;
      });
      return {
        targets: foes(a).map(function (x) { return x.name; }),
        forms: forms,
        open: !a.open && a.coolLock === 0 && a.drive >= T.open.cost,
        extra: a.form === 'mobility' && !a.open && a.drive >= T.form.mobility.extraCost && (T.overheat.extra || !a.overheated),
      };
    }
    function reserve(a, cmd) { a.reserved = cmd || null; }
    return { actors: all, log: log, step: step, result: result, can: can, reserve: reserve,
             beat: function () { return beat; }, over: function () { return over; }, T: T };
  }

  function battle(sideA, sideB, opt) {
    var g = createBattle(sideA, sideB, opt);
    while (!g.step()) {}
    return g.result();
  }
  ...
  function managedWith(p) {
    ...
    return function (a, s) {
      var T = s.T, work = (a.nature && a.nature !== 'light') ? a.nature : (a.stats.def >= a.stats.spe ? 'heavy' : 'mobility');   // 일하는 폼 — 성격이 있으면 그것 (경장형은 능력치대로)
  ...
  return { TUNING: TUNING, POLICY: POLICY, MANAGED: MANAGED, managedWith: managedWith, rng: rng, budgetStats: budgetStats, makeActor: makeActor,
           NATURE: NATURE, applyNature: applyNature, fitNature: fitNature,
           battle: battle, createBattle: createBattle, typeMul: typeMul, merge: merge };
});
