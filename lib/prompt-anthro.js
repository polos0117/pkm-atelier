...
(function(root){
  ...
  function referenceActionMounts(st, source) {
    ...
    if(out.length) out.push(R.referenceActionOcclusion);   /* 등 장비가 있을 때만 — 보이게 하려고 옮기는 것을 막는다 */
    return out.join('\n');
  }
  ...
  function referenceActionDetails(st, rearGear) {
    var details=[];
    /* 등 장비 종은 사용자가 방향·자세·카메라를 안 정했을 때 돌아보는 3/4 후면이 기본 —
       정면에서 "장비도 보이게" 하면 모델이 등딱지를 어깨로 옮겨 온다 (세 번 겪었다) */
    if(rearGear&&!['orient','pose','camera','frame'].some(function(k){return clean(st[k])})) details.push(R.referenceActionRearPose);
