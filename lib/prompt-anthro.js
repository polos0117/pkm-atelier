...
(function(root){
  ...
  function featureInsetSuggestions(st) {
    ...
    var automatic=!clean(st.motifs)&&sourceFeatures(st);
    if(automatic&&automatic.length) {
      var targets=automatic.slice(0,3);
      if(targets.length<3&&clean(st.sourceAppearance.color)) targets.push(st.sourceAppearance.color+' source palette');
      if(targets.length<3&&clean(st.sourceAppearance.shape)) targets.push(st.sourceAppearance.shape+' source silhouette');
      while(targets.length<3) targets.push('one distinct existing source-derived surface detail');
      return {face:face.join(', ')||'face identity from CHARACTER IDENTITY: eyes, hairline, hairstyle and jaw/chin',
        front:targets[0],rear:targets[1],function:targets[2]};
    }
    var clauses=clean(st.motifs).split(/\s*(?:[,;\n]|\s\/\s)\s*/).map(clean).filter(Boolean);
    var terms=S.FEATURE_INSET_TERMS;
    function ranked(kind) {
      return clauses.map(function(text,index){
        var lower=text.toLowerCase(),score=terms[kind].reduce(function(n,term){return n+(lower.indexOf(term)>=0?1:0)},0);
        return {text:text,index:index,score:score};
      }).filter(function(row){return row.score>0}).sort(function(a,b){return b.score-a.score||a.index-b.index});
    }
    var used=[];
    function pick(kind,fallback) {
      var options=ranked(kind).map(function(row){return row.text}).concat(clauses);
      var chosen=options.find(function(t){return used.indexOf(t)<0});
      if(chosen) used.push(chosen);
      return chosen||fallback;
    }
    var front=pick('front','primary source component from the design record');
    var rear=pick('rear','second distinct source component from the design record');
    var functional=pick('function','third distinct source component or surface from the design record');
    return {
      face:face.join(', ')||'face identity from CHARACTER IDENTITY: eyes, hairline, hairstyle and jaw/chin',
      front:front,rear:rear,function:functional
    };
  }
  function featureInsetBlock(st) {
    ...
    return '1. FACE IDENTITY — '+get('face')+'.\n'+
      '2. SIGNATURE DETAIL A — '+get('front')+'.\n'+
      '3. SIGNATURE DETAIL B — '+get('rear')+'.\n'+
      '4. SIGNATURE DETAIL C — '+get('function')+'.\n'+
  ...
  function referenceActionForm(st, form) {
    ...
    if(form!=='overdrive') text+=S.ACTION_FORM_PROFILES[form];
      ...
      text+='OVERDRIVE BASE: '+(base==='reference'?
        ...
        'OVERDRIVE: open selected existing panels at their established hinges, reveal the inner frame and source-appropriate energy, and show physical strain through the pose. Keep the base armor inventory, attachment points, colors and mass recognizable.';
  ...
  function buildReferenceAction(st, source, style, form) {
    ...
    add('SOURCE ENGINEERING',R.referenceActionDesign+(mounts?'\n'+mounts:''));
  ...
  root.AtelierPrompt={
    ...
    featureInsetSuggestions:featureInsetSuggestions,
