...
(function(root){
  ...
  function featureInsetSuggestions(st) {
    ...
    var front=ranked('front')[0]?.text||'front torso signature and attachment';
    var rearRows=ranked('rear'),rear=rearRows.slice(0,2).map(function(row){return row.text}).join(' + ')||
      'rear-mounted signature and attachment';
    var functional=ranked('function')[0]?.text||'working mechanism, housing and attachment';
    return {
  ...
  function featureInsetBlock(st) {
    ...
    return '1. FACE IDENTITY — '+get('face')+'.\n'+
      '2. FRONT SIGNATURE — '+get('front')+'.\n'+
      '3. REAR MOUNTING SYSTEM — '+get('rear')+'.\n'+
      '4. FUNCTIONAL COMPONENT — '+get('function')+'.\n'+
