...
(function(root){
  ...
  function buildPrompt(st) {
    ...
    if(mode!=='casual') {
      ...
      add('FORM DEFINITION',R.formCommon+'\n\n'+definition);
    ...
    add('OUTPUT MODE',S.OUTPUT_PROFILES[mode]+(details.length?'\n\nScene details (within identity and mode boundaries):\n'+details.join('\n'):''));
    add('CAMERA & PRESENTATION',mode==='portrait'?R.portraitCamera:
      R.freeCamera+'\nAspect ratio: '+(clean(st.aspect)||'2:3')+
      (clean(st.frame)?'\nFraming: '+clean(st.frame):'')+
      (clean(st.camera)?'\nCamera: '+clean(st.camera):''));
    add('CONSISTENCY / NEGATIVE LOCK',R.negative);
    add('FINAL CHECK',R.final);
    return blocks.join('\n\n');
  }
