...
const defaults=()=>({style:S.DEFAULT_STYLE,identityMode:'create',form:'light',baseForm:'light',
 motifs:'',formOverride:'',featureInsets:{},params:{},locks:{},mode:'portrait',
...
function normalize(input){
 ...
 for(const k of ['face','front','rear','function'])if(typeof out.featureInsets[k]!=='string')out.featureInsets[k]='';
...
function Editor({card,images,appearance}){
 ...
 const insetSuggestions=P.featureInsetSuggestions({params:st.params,motifs:st.motifs,sourceAppearance:appearance});
 ...
 return h('div',{class:'studio-editor'},
  ...
  mode==='portrait'&&!reference&&h('section',{class:'studio-card',id:'feature-insets'},
   ...
   ['face','front','rear','function'].map(key=>h(Text,{key,id:'feature-inset-'+key,
