...
import {AppearanceControls,WorkspaceHeading} from './workspace-ui.js?v=pokemon1';
...
const blankScene=()=>({expr:'',pose:'',orient:'',scene:'',outfit:'',camera:'',custom:'',aspect:'2:3',
 cat:'everyday_basic',catCustom:'',ex:'',exCustom:'',axes:{}});
...
function Select({id,label,value,options,onChange}){
 return h('label',{class:'studio-field'},h('span',null,label),
  h('select',{id,value,onChange:e=>onChange(e.target.value)},
   options.map(([v,t])=>h('option',{key:v,value:v},t))));
}
function Text({id,label,value,onChange,multiline=false,placeholder=''}){
 return h('label',{class:'studio-field'},h('span',null,label),
...
function Param({d,value,locked,onValue,onLock}){
 const options=d.options.map(([v,t])=>[v,v===''?W('prompt.auto'):t]);
 const custom=value&&!options.some(([v])=>v===value),selected=custom?'__custom__':value||'';
 return h('div',{class:'studio-param'},
...
function Editor({card,images}){
 ...
 const sceneSelect=(id,key,options,label)=>h(Select,{id,label:W('prompt.'+(label||key)),value:scene[key],
  options,onChange:v=>setScene(key,v)});
 const sceneText=key=>h(Text,{key,id:key,label:W(key==='custom'?'prompt.extra':'prompt.'+key),
  value:scene[key],multiline:true,onChange:v=>setScene(key,v)});
 ...
 return h('div',{class:'studio-editor'},
  ...
  h('section',{class:'studio-card'},
   ...
   reference&&note(W('prompt.anchor.note')),note(W('prompt.'+mode+'.note')),
  ...
  h('section',{class:'studio-card'},h('h2',{class:'studio-title'},W('prompt.scene.settings')),
   ...
   mode!=='portrait'&&h('div',null,
    mode==='casual'&&h('div',{class:'studio-grid-fields'},
     ...
     sceneSelect('example','ex',examples,'example'),
     scene.cat==='__custom__'&&h(Text,{id:'category-custom',label:W('prompt.custom'),
      value:scene.catCustom,onChange:v=>setScene('catCustom',v)}),
     scene.ex==='__custom__'&&h(Text,{id:'example-custom',label:W('prompt.custom'),
      value:scene.exCustom,onChange:v=>setScene('exCustom',v)})),
    h('div',{class:'studio-grid-fields'},
     sceneSelect('pose','pose',S.POSE_OPTIONS),
     ...
     h(Text,{id:'camera',label:W('prompt.camera'),value:scene.camera,onChange:v=>setScene('camera',v)})),
...
function App(){
 ...
 return h('div',null,
  h(WorkspaceHeading,{title:W('prompt.title'),subtitle:W('prompt.subtitle'),code:W('app.code')}),
  h('nav',{class:'workspace-nav'},h('a',{href:'index.html'},W('nav.index')),
   h('a',{href:'dex.html'},W('nav.dex')),h('a',{href:'prompt.html','aria-current':'page'},W('prompt.title'))),
