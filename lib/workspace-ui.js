...
export function WorkspaceHeader({page}) {
  const W=window.W;
  const title={index:'app.title',dex:'dex.title',prompt:'prompt.title',battle:'battle.title',run:'run.title'}[page];
  return h('div',{class:'workspace-header'},
    h(WorkspaceHeading,{title:W(title),code:W('app.code'),
      subtitle:W(page==='prompt'?'prompt.subtitle':'app.subtitle')}),
    h('nav',{class:'workspace-nav'},['index','dex','prompt','battle','run'].map(key=>
      h('a',{key,href:key+'.html','aria-current':key===page?'page':undefined},
        W(key==='prompt'?'prompt.title':'nav.'+key)))),
