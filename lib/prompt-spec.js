...
(function (root) {
  ...
  const ACTION_FORM_PROFILES = {
    "light": "LIGHT: fit one compact protective layer at the shoulders, arms, hips or thighs and lower legs. Use clear joints, beveled plate depth and flexible gaps for an agile regional silhouette.",
    "heavy": "HEAVY: enclose the unchanged reference body with layered armor around chest, back, ribs, abdomen, waist, pelvis, upper arms, forearms, thighs, knees, shins and feet. Use a segmented waist, wraparound cuisses, substantial shoulder and forearm shells and deep load-bearing greaves. The larger silhouette comes from shell thickness and clearance around the original body; the face remains visible.",
    "mobility": "MOBILITY: arrange thin armor into an outward, swept-back silhouette with directional panels, compact propulsion structures and clear negative space. Keep joints free and make the silhouette visibly different from compact light armor at thumbnail size."
  };
  ...
  const REFERENCE_SHEET_PROFILE = {
    "output": "INITIAL CHARACTER REFERENCE SHEET: same individual and same selected armor configuration in every view. Layout: large front full-body view left; equal-scale rear three-quarter full-body view center; exactly four detail insets in a narrow right column following FEATURE INSET LIST top to bottom. Document one design, not variants.",
    "featureRule": "A/B/C each document their resolved component or surface region and relevant connection. No mandatory backpack or weapon. Explicit user targets take priority. Generic armor/full-body repeats do not count.",
    ...
    "negative": "INSET FIDELITY LOCK: Each inset magnifies its named installed component or surface region, never a new design. Match main-view silhouette, panels, colors, markings, channels, fasteners and mounts; finer detail may increase, structure may not. Plan details before views. Keep face, hair, anatomy, coverage and left/right identity identical. Each full body has two arms and two legs; appendages are equipment. If tailed, the rear three-quarter view must show the posterior sacral attachment; a tail inset, when selected, documents that same connection. No alternate faces/outfits, extra limbs, childlike proportions or text overlays.",
    "final": "Verify both full-body views and all four named matching detail insets, readable face and attachments, consistent anatomy and selected style. This sheet becomes an identity anchor only after user approval."
};
  const FEATURE_INSET_TERMS = {
    "front": ["chest", "breast", "torso", "plastron", "sternum", "abdomen", "belly", "collar", "bib", "front plate", "가슴", "흉부", "플라스트론", "복부", "배판", "앞판", "칼라"],
    "rear": ["back", "rear", "dorsal", "shell", "carapace", "backpack", "bulb", "seed", "tail", "wing", "spine", "등", "후면", "등껍질", "갑각", "백팩", "구근", "씨앗", "꼬리", "날개", "척추"],
    "function": ["nozzle", "jet", "thruster", "vent", "emitter", "cannon", "turbine", "engine", "reactor", "cable", "claw", "blade", "sensor", "propuls", "port", "fin", "노즐", "제트", "추진", "배기", "방출", "포구", "터빈", "엔진", "리액터", "케이블", "집게", "칼날", "센서", "포트", "분사", "핀"]
  };
  const PARAM_GUIDES = {
    "body type": {
  ...
  root.AtelierSpec = {
    ...
    PROJECT_RULES, FORM_PROFILES, ACTION_FORM_PROFILES, ACTION_MOUNT_HINTS, ACTION_STYLE_CORES, OUTPUT_PROFILES, REFERENCE_SHEET_PROFILE, FEATURE_INSET_TERMS, IDENTITY_GROUPS, IDENTITY_EXCLUDED,
