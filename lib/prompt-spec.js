...
(function (root) {
  ...
  "sourceDesignReference": "SOURCE-TO-MECHANISM DESIGN: Preserve the approved source-to-mechanism design, signature equipment, colors and markings. MOUNT CORRECTION PRIORITY: Reconstruct attachment geometry using BODY-SPACE MOUNT LOCK, even when continuing the same form. This correction overrides attachment/silhouette preservation in reference, form and style instructions; retain the person, component inventory and surface design. Image approval alone does not approve an ambiguous attachment. Do not copy a reference overlap as a side-mounted component. Allocate visual mass in source-defining external equipment. Apply the selected form through its external construction; overdrive opens the existing base equipment.",
  "sourceMounts": "BODY-SPACE MOUNT LOCK: For present or chosen equipment only; do not invent a shell, tail or thrusters. Center a dorsal shell cradle on the spinal midline, inner face following the back and dome facing rearward; it follows the torso, not the camera. A tail has an independent sacral root on the posterior pelvic midline below the shell, extending rearward before curling. Keep shell cradle, tail root and propulsion nozzles distinct, not one shell-edge exhaust. Never attach the tail to the side waist, hip plate or arm. The curl follows the tail in 3D, not a camera-facing disk; do not swing the whole curl beside an elbow to display its spiral. Natural occlusion by the torso or arms is correct; front views may hide rear equipment entirely. Keeping equipment in frame forbids cropping, not occlusion. Do not slide it toward a shoulder or flank, rotate it toward the viewer, duplicate it, or alter framing, margins or anatomy to display it. Light, heavy, mobility and overdrive change authorized shells, fins and panel articulation, but not relocate the main mounts; components move around their own joints. Continuations retain body-relative mounts, not rear-view screen coordinates. Use consistent rear views to resolve overlap; never infer a side mount from a hidden root. On sheets, the existing rear view and equipment inset document both the shell cradle and separate tail root when present. Only an explicit user instruction for alternative mounts overrides this geometry.",
