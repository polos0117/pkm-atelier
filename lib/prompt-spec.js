...
'traditional':'그 캐릭터의 계통에 맞는 전통 의상. 정장형과 현대화된 일상형 중에서 고른다. 고증을 지키되 색은 원본 포켓몬를 따를 수 있다.'};
  ...
  const ANTHRO_STYLE_EXTENSIONS = {
    ...
    game_keyart: [
      ...
      'REFERENCE FIDELITY: render the identity and selected form defined below through this style; do not copy the reference image\'s own rendering language \u2014 its cel-animation, photographic, or CGI look \u2014 into the final image.',
  ...
  "formCommon": "The same human body sits beneath every form. Form changes affect armor layering, thickness, arrangement and visible bodysuit, not height, bust, waist, hips, limb lengths or muscle mass. Keep source-derived colors and recognizable structural motifs. Preserve attached design details where compatible with the selected form; armor is not an immutable identity lock.",
  ...
  "final": "Before finishing, verify one adult woman, the chosen source identity, the correct output mode, coherent anatomy and the selected rendering style. For reference generation compare face, hair and body with the approved identity anchor. For armored modes verify the chosen layering or opening logic without changing the body. For casual scenes verify that no form rules leaked into the outfit. Obey identity and mode boundaries even when a local detail request conflicts with them.",
